...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  with tf.name_scope(layer_name):
    ...
    #with tf.name_scope('hidden1_biases'):
     # layer1_biases = tf.Variable(tf.zeros([1024]), name='hidden1_biases')
      #variable_summaries(layer1_biases)
    with tf.name_scope('hidden1_Wx_plus_b'):
      #logits1 = tf.matmul(bottleneck_input, layer1_weights) + layer1_biases
    ...
    with tf.name_scope('hidden1_activation'):
      ...
      tf.summary.histogram('hidden1_Activation',inp_activated)
    with tf.name_scope('hidden1_dropout'):
      dropped_hidden1=tf.layers.dropout(inp_activated,rate=0.4,training=True,name='hidden1_dropout_layer')
      variable_summaries(dropped_hidden1)

    #define hidden layer 2
    with tf.name_scope('hidden2_weights'):
      hidden2_weight = tf.truncated_normal([1024, 1024], stddev=0.001)
      layer2_weights = tf.Variable(hidden2_weight,name='hidden2_weights')
      variable_summaries(layer2_weights)
    #with tf.name_scope('hidden2_biases'):
     # layer2_biases = tf.Variable(tf.zeros([1024]),name='hidden2_biases')
      #variable_summaries(layer2_biases)
    with tf.name_scope('hidden2_Wx_plus_b'):
    ...
    with tf.name_scope('hidden2_batchNorm'):
     BN2_logits = tf.layers.batch_norm(logits_hidden2,
					       center=True,
					       scale=True,
					       is_training=training,
    ...
    with tf.name_scope('hidden2_dropout'):
      dropped_hidden2=tf.layers.dropout(hidden2_activated,rate=0.5,training=True,name='hidden2_dropout_layer')
      variable_summaries(dropped_hidden2)
    ...
    with tf.name_scope('output_Wx_plus_b'):
      logits = tf.matmul(dropped_hidden2, layer_weights) + layer_biases
      tf.summary.histogram('pre_activations', logits)
    #with tf.name_scope('dropout'):
	#	drops=tf.layers.dropout(logits,rate=0.4,training=True,name='final_dropout_layer')
	#	variable_summaries(drops)
...
def add_evaluation_step(result_tensor, ground_truth_tensor):
  ...
  ground_truth_preds=tf.argmax(ground_truth_tensor, 1)
...
#  img_d_summary_dir = os.path.join(checkpoint_dir, "summaries")
#  img_d_summary_writer = tf.summary.FileWriter(img_d_summary_dir, sess.graph)
  #confusion_matrix_summary = plot_confusion_matrix(tf.argmax(ground_truth_tensor,1), prediction,labels=image_labels, tensor_name='dev/cm')
  #img_d_summary_writer.add_summary(img_d_summary, global_step=1)

  
  return evaluation_step, prediction,ground_truth_preds
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    evaluation_step, prediction,ground_truth_preds = add_evaluation_step(
    ...
    conf_writer = tf.summary.FileWriter(
        FLAGS.summaries_dir+'/confusion_matrix')
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        ...
        confMat_summary=plot_confusion_matrix(correct_labels=g_truths,predict_labels=predicts,labels=image_labels,session=sess)
        conf_writer.add_summary(confMat_summary,i)
